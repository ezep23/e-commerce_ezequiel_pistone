...
const finishOrder = document.querySelector("#processCheckout");
...
finishOrder.addEventListener("click", async () => {
    const data = { items: JSON.parse(localStorage.getItem("cart")) }; 
    ...
    try { 
        ...
        if(response.ok){
            window.location.href = products.payment_url;
        } else{
            alert(data.error)
        }
